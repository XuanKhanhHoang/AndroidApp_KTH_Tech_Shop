...
public class UserEditInfoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.Global_BackBtn).setOnClickListener(v -> {
            finish();
        });
        ...
        apiServiceObject.apiServices.getCustomerDetail(accessToken).enqueue(new Callback<UserFullDetail>() {
            ...
            public void onResponse(@NonNull Call<UserFullDetail> call, @NonNull Response<UserFullDetail> response) {
                if (response.isSuccessful()) {
                    ...
                    firstNameTxtILayout.getEditText().setText(bd.firstName);
                    lastNameTxtILayout.getEditText().setText(bd.lastName);
                    userNameTxtILayout.getEditText().setText(bd.loginName);
                    genderRadioGr.check(bd.gender ? R.id.UserEditInfoActivity_GenderMale : R.id.UserEditInfoActivity_GenderFemale);
                    emailTxtILayout.getEditText().setText(bd.email);
                    phoneNumberTxtILayout.getEditText().setText(bd.phoneNumber);
                    addressTxtILayout.getEditText().setText(bd.address);
            ...
            new ApiServiceObject().apiServices.editUser(accessToken, data).enqueue(new Callback<updateUserDetailResponse>() {
                ...
                public void onResponse(@NonNull Call<updateUserDetailResponse> call, @NonNull Response<updateUserDetailResponse> response) {
                    if (response.isSuccessful()) {
                        ...
                        Intent it = new Intent(UserEditInfoActivity.this, UserActivity.class);
                        UserEditInfoActivity.this.startActivity(it);
                ...
                @Override
                public void onFailure(Call<updateUserDetailResponse> call, Throwable t) {
        ...
        switch (requestCode) {
            case MY_PERMISSIONS_REQUEST_READ_EXTERNAL_STORAGE: {
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    // Quyền đã được cấp, tiếp tục hành động của bạn
                } else {
    ...
    private boolean validateInput(TextInputLayout... layouts) {
        ...
        String email = emailTxtILayout.getEditText().getText().toString();
        String phoneNumber = phoneNumberTxtILayout.getEditText().getText().toString();
